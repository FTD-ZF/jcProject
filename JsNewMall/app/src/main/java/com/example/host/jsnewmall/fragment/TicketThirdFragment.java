...
import com.example.host.jsnewmall.adapter.TicketThirdAdapter;

/**
 * Created by host on 2017/2/22.
 */

public class TicketThirdFragment extends BaseFragment {
    private View view;
    ...
    private void initView(){
        ...
        mNetImageFirst.setBackground(getActivity().getResources().getDrawable(R.mipmap.icon_ticket_third_banner));

        mTvFirstContent.setText("\u3000\u3000"+getActivity().getResources().getString(R.string.ticket_a_c)
                +"\n"
        +"\u3000\u3000"+getActivity().getResources().getString(R.string.ticket_a_d));

        mFirstListView=(ListView) view.findViewById(R.id.list_hotel_view);  //图片显示列表
        mFirstListView.addHeaderView(headerView);
        TicketThirdAdapter mFirstAdapter=new TicketThirdAdapter(getActivity(),queue);
