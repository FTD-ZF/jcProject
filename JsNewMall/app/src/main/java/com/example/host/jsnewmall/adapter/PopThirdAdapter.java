...
public class PopThirdAdapter extends BaseAdapter {

    private SearchkeywordEntry mBodyinfo;
    private Context mContext;
    private int mSelect=0;

    public PopThirdAdapter(Context context, SearchkeywordEntry info){
        this.mBodyinfo=info;
        this.mContext=context;
    }
    @Override
    public int getCount() {
        return mBodyinfo.getProductdata().size();
    }

    @Override
    public Object getItem(int position) {
        return mBodyinfo.getProductdata().get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        holder.tvname.setText(mBodyinfo.getProductdata().get(position).getAttrdetailname());
