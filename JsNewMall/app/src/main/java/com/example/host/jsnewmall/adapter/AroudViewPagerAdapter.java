...
public class AroudViewPagerAdapter extends PagerAdapter {
    ...
    public AroudViewPagerAdapter(Context context, RequestQueue queue, TravelBsEntry bodyinfo) {
        super();
...
//        return mBodyInfo.getList().getBanner().size();
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        if (mBodyInfo.getList().getBanner().size()!=0) {
            final TravelBsEntry.ListBean.BannerBean bannerInfo = mBodyInfo.getList().getBanner().get(position
